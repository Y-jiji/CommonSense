...
#define PWD "/media/mydrive/CDMA/build/3rd_party_download/Jeffery-Meng/IBLT-opt"

#ifdef PWD

// parameter file is updated, but here is not.
std::string IBLT::parameter_file = PWD "/param.export.0.995833.2018-07-17.csv";
...
static uint64_t MurmurHashTwice(uint64_t nHashSeed, const std::vector<unsigned char>& vDataToHash) {
    static uint64_t bucket[2] = {0, 0};
    auto reseed = ToVec(nHashSeed);
    MurmurHash3_x64_128(&reseed[0], reseed.size(), nHashSeed, &bucket[0]);
    nHashSeed = bucket[0] ^ bucket[1];
    MurmurHash3_x64_128(&vDataToHash[0], vDataToHash.size(), nHashSeed, &bucket[0]);
